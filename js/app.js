...
btnusers.addEventListener('click',()=>{
    ...
    .then(json => {
       ...
       document.getElementById('mnusers').innerHTML=opciones;
    })
})

const mnusers=document.getElementById("mnusers");
mnusers.addEventListener("change",()=>{
    let v1=document.getElementById("mnusers").value;
    v1=parseInt(v1);
    ...
    .then(json=>{
        ...
        <label for='txtEmpresa'><input type='text' id='txtEmpresa' value='${json.company.name}></label>`;
        document.getElementById("userdetail").innerHTML=res;
    })*/
})
...
function cargarcoments(blog){
    ...
    .then(json => {
        ...
        json.forEach(user => {
            ...
            comentarios+=`<option value="${user.id}">
            <p>${user.email}:</p><br>
            <p>${user.body}</p></option><br>`
        });
